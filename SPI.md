...
### Operating Modes (CPOL & CPHA)     
SPI supports four modes based on Clock Polarity (CPOL) and Clock Phase (CPHA), determining if data is sampled on the rising or falling edge.
- **Mode 0 (Default):** CPOL=0 (idle low), CPHA=0 (sample on rising edge).
- **Mode 3:** CPOL=1 (idle high), CPHA=1 (sample on rising edge).

### Master-Slave Configuration:    
The master initiates communication by pulling the CS line low for a specific device.
