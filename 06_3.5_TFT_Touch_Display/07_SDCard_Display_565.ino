...
#define FILE_EXT "b565"
#define FILE_EXT_TXT "565"

/**********************************************************************************************
Three STREAM_DIRECTLY methods to display the image from file 565 or b565
Method 1: Display image line by line
Method 2: Display image in chunked, defined by memory allocation example 100*1024
Method 3: Display image directly from text 565 file, make sure switch file extension to FILE_EXT_TXT above
*********************************************************************************************/
...
bool displayBinary565(const char* filename, TFT_eSPI& tft, int destX, int destY);
bool displayBinary565Chunked(const char* filename, TFT_eSPI& tft, int destX, int destY, size_t maxChunkSize);
bool display565FileDirect(const char* filename, TFT_eSPI& tft, int destX, int destY);
...
void setup() {
  ...
  if (files && fileCount > 0) {
    for (int i = 0; i < fileCount; i++) {
      ...
      /**
Three method to display the image from file 565 or b565
**/

#if METHOD == 1
      // Method 1: Display image line by line
      if (displayBinary565(files[i].c_str(), tft, -1, -1)) {
#elif METHOD == 2
      // Method 2: Display image in chunked, defined by memory allocation example 100*1024
      if (displayBinary565Chunked(files[i].c_str(), tft, -1, -1, 100 * 1024)) {
#else
      // Method 3: Display image directly from text 565 file, make sure switch file extension to FILE_EXT_TXT above
      if (display565FileDirect(files[i].c_str(), tft, -1, -1)) {
#endif
        // Serial.println("  OK");
      } else {
    ...
    Serial.println("No .b565 files found");
  }

#else
  /**
  Copy RGB565 file contains from SD card and than display to TFT
...
**/
  ...
  if (files && fileCount > 0) {
    for (int i = 0; i < fileCount; i++) {
      ...
      // Read 565 image file than display
      if (read565FileText(files[i].c_str(), height, width, pixels)) {
    ...
    Serial.println("No .565 files found or SD error.");
  }
...
/**
 * Reads a .565 file from the SD card and extracts the image dimensions
 * and pixel data.
 *
 * @param filename  Full path to the .565 file (e.g., "/images/img.565").
 ...
 *                  size mismatch, etc.).
 */
bool read565File(const char* filename, uint16_t& height, uint16_t& width, uint16_t*& data) {
...
/**
 * Reads a .565 file in text format:
 *   height,width,{ 0xXXXX, 0xXXXX, ... };
 *
 * @param filename  Full path (e.g., "/Shangrila.565")
 ...
 */
bool read565FileText(const char* filename, uint16_t& height, uint16_t& width, uint16_t*& data) {
...
/**
 * Reads a .565 text file directly from SD and displays it on the TFT,
 * row by row, without storing the whole image in RAM.
 * 
 * @param filename   Full path to the .565 file (e.g., "/Shangrila.565")
...
bool display565FileDirect(const char* filename, TFT_eSPI& tft, int destX = -1, int destY = -1) {
  File file = SD.open(filename, FILE_READ);
  if (!file) {
...
bool displayBinary565(const char* filename, TFT_eSPI& tft, int destX = -1, int destY = -1) {
  File file = SD.open(filename, FILE_READ);
  if (!file) {
    Serial.println("Cannot open file");
    return false;
...
/**
 * Display a binary .b565 file using a chunked buffer (max ~100KB).
 * Falls back to full‑buffer if the entire image fits within the limit.
 */
bool displayBinary565Chunked(const char* filename, TFT_eSPI& tft, int destX = -1, int destY = -1, size_t maxChunkSize = 100 * 1024) {
  File file = SD.open(filename, FILE_READ);
  if (!file) return false;
  ...
  if (bufferSize <= maxChunkSize) {
    file.close();
    return displayBinary565(filename, tft, destX, destY);  // reuse your full‑buffer version
  }
